...
def test_address_one_df(df_samples):
    ...
    assert pd.Series(['address_df_diff','address_df_similar_diff']).isin(in_cluster.columns).all()
    assert len(out_cluster)==4
    assert pd.Series(['address_df_diff','address_df_similar_diff']).isin(out_cluster.columns).all()
...
def test_address_two_df(df_samples):
    ...
    assert pd.Series(['address_df_diff','address_df2_similar_diff']).isin(in_cluster.columns).all()
    assert len(out_cluster)==1
    assert pd.Series(['address_df_diff','address_df2_similar_diff']).isin(out_cluster.columns).all()